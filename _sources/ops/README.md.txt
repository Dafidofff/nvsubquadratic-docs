...
| {ref}`fftconv_fp16.py <ops-fftconv-fp16>`                   | fp16      | linear        | depthwise      | Memory/throughput savings on power-of-2 spatial dims. Drop-in for `fftconv.py`.                                                                                                                                                                       |
| {ref}`circular_fftconv_fp16.py <ops-circular-fftconv-fp16>` | fp16      | circular      | depthwise      | Same as above for the circular case. Uses **dual mean-centering** for fp16 stability — see the [FP16 circular FFT convolution report](https://github.com/NVIDIA-Digital-Bio/nvSubquadratic-private/blob/main/reports/fp16_fft_convolution/REPORT.md). |
...
1. **What's my precision budget?**
   ...
   - For aggressive memory/throughput savings on **power-of-2 spatial dims**, use the `*_fp16_*` variant. The fp16 ops use `norm="ortho"` and (for circular) dual mean-centering to stay within fp16 dynamic range — see the [FP16 circular FFT convolution report](https://github.com/NVIDIA-Digital-Bio/nvSubquadratic-private/blob/main/reports/fp16_fft_convolution/REPORT.md).
